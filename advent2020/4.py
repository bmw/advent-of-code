...
"""
invalid_example = """eyr:1972 cid:100
...
byr:1965
"""
valid_example = """pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
HAIR_REGEX = re.compile('#[a-f0-9]{6}')
...
def is_valid(passport):
    ...
    if HAIR_REGEX.fullmatch(passport['hcl']) is None:
        return False

    if not any(color == passport['ecl'] for color in 'amb blu brn gry grn hzl oth'.split()):
        return False

    if PID_REGEX.fullmatch(passport['pid']) is None:
        return False

    return True

print(sum(1 for passport in passport_generator() if is_valid(passport)))
