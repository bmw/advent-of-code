...
fn seeds_to_test2(seed_ranges: &SeedRanges, map: &NumMap) -> Vec<u64> {
    let mut v = Vec::new();
...
fn compare_maps(old_map: &Map, new_map: &NumMap) {
    let mut tmp_map = NumMap::default();
    // how to make collect work for my map?
    for &MapRange { dest_start, src_start, len } in old_map {
        tmp_map.insert(src_start, dest_start, len);
    }
    assert_eq!(tmp_map, *new_map);
}
...
fn do_part<T: Fn(&str) -> SeedRanges>(input: &str, seed_fn: T) -> u64 {
    ...
    let mut new_map = NumMap::default();
    ...
    while iter.peek().is_some() {
        let map_lines: Vec<&str> = iter
            ...
            .collect();
        old_map = update_map(&old_map, &map_lines);
        let tmp_map = NumMap::try_from(&map_lines).unwrap();
        new_map = new_map.merged_maps(&tmp_map);
        compare_maps(&old_map, &new_map);
    }
    let seeds = seeds_to_test2(&seed_ranges, &new_map);
    seeds.iter().map(|&v| new_map.map_value(v)).min().unwrap()
    //let mut seeds = seeds_to_test(&seed_ranges, &old_map);
    //apply_map(&old_map, &mut seeds);
